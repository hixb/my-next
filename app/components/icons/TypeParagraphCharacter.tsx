export default function TypeParagraphCharacter() {
  const link21 = () => (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
      xmlns="http://www.w3.org/2000/svg">
      <path
        d="M13.0598 10.9414C15.3098 13.1914 15.3098 16.8314 13.0598 19.0714C10.8098 21.3114 7.16985 21.3214 4.92985 19.0714C2.68985 16.8214 2.67985 13.1814 4.92985 10.9414"
        stroke="#292D32" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
      <path className="set-svg-stroke"
        d="M10.5909 13.4089C8.25094 11.0689 8.25094 7.26891 10.5909 4.91891C12.9309 2.56891 16.7309 2.57891 19.0809 4.91891C21.4309 7.25891 21.4209 11.0589 19.0809 13.4089"
        stroke="#292D32" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  )

  return {
    link21,
  }
}
